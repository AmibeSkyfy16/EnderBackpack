...
public class BackpacksManager {
     ...
     */
    public static void getCorrectInventorySize(PlayerEntity player, Long time) {
        ...
        Configurator.getInstance().config.sizes.forEach((keyTime, row) -> {
            ...
            if (timeIsGreaterThan.get() && count.get() == Configurator.getInstance().config.sizes.size() - 1) {
                ...
                // Only to do line if on dedicated server
                if(player instanceof ServerPlayerEntity serverPlayerEntity) {
                    sendDataToPlayer(serverPlayerEntity, row);
                }
            ...
            if (time >= keyTime)
                ...
                if (timeIsGreaterThan.get()) {
                    ...
                    // Only to do line if on dedicated server
                    if(player instanceof ServerPlayerEntity serverPlayerEntity) {
                        sendDataToPlayer(serverPlayerEntity, lastRow.get());
                    }
                }
